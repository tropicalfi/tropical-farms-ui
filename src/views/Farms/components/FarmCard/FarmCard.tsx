...
import { Tag, VerifiedIcon, Flex, Text, Skeleton, HelpIcon } from '@pancakeswap-libs/uikit'
...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethPrice, account }) => {
  ...
  const lpTokenPriceFormated = lpPrice
    ...
    const lpTokenPriceFormated2 = lpPrice

  const lpLabel = farm.lpSymbol && farm.lpSymbol.toUpperCase().replace('PANCAKE', '')
  const earnLabel = farm.dual ? farm.dual.earnLabel : 'DAIQUIRI'
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'DAIQUIRI' && <StyledCardAccent />}
      ...
      <Flex justifyContent="space-between">
        ...
        <Text bold>{earnLabel} {farm.isTokenOnly ? "" : " + Fees"}</Text>
      </Flex>
      <Flex justifyContent="space-between">
      ...
      <CardActionsContainer lpPrice={lpTokenPriceFormated2} farm={farm} account={account} addLiquidityUrl={addLiquidityUrl} />
      <Divider />
      ...
      <ExpandingWrapper expanded={showExpandableSection}>
        <DetailsSection
          removed={removed}
          bscScanAddress={!farm.isTokenOnly ? `https://smartscan.cash/address/${farm.lpAddresses[process.env.REACT_APP_CHAIN_ID]}`
        : `https://smartscan.cash/address/${farm.tokenAddresses[process.env.REACT_APP_CHAIN_ID]}`}
